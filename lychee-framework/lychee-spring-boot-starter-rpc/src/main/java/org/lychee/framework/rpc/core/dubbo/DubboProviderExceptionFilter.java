package org.lychee.framework.rpc.core.dubbo;
...
import org.lychee.common.exception.ServerException;
...
import static org.lychee.common.exception.constants.GlobalErrorCodeConstants.BAD_REQUEST;
import static org.lychee.common.exception.constants.GlobalErrorCodeConstants.INTERNAL_SERVER_ERROR;
...
public class DubboProviderExceptionFilter implements Filter, Filter.Listener {
    ...
    public void onResponse(Result appResponse, Invoker<?> invoker, Invocation invocation) {
        if (appResponse.hasException() && GenericService.class != invoker.getInterface()) {
            try {
                ...
                // 2.1 如果是 ServiceException 异常，并且返回参数类型是 R 的情况，则将转换成 R 返回
                if (isReturnR(invocation) && exception instanceof ServiceException) {
                    appResponse.setException(null); // 一定要清空异常
                    appResponse.setValue(R.error((ServiceException) exception));
    ...
    private boolean isReturnR(Invocation invocation) {
